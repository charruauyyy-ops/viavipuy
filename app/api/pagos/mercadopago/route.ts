...
export async function POST(req: NextRequest) {
  try {
    ...
    if (tipo === "publicacion") {
      ...
      if (!publicacion_id || !duracion_dias || !monto) return NextResponse.json({ error: "Faltan campos para publicacion" }, { status: 400 });
      ...
      if (!plan_id || !duracion_dias) return NextResponse.json({ error: "Faltan campos para plan" }, { status: 400 });
      ...
      monto = planMonto;
      planNombre = (PLANS as any)?.[plan_id]?.name || (PLANS as any)?.[plan_id]?.titulo || plan_id;
    ...
    const query = sc.from("pagos_viavip")
    ...
    if (pending?.id) {
      pagoId = pending.id;
      reused = true;
      const updateData: any = { metodo_pago: "mercadopago", moneda: "UYU", estado: "pendiente", estado_pago: "pendiente" };
      if (tipo === "publicacion") {
    ...
    return NextResponse.json({ pago_id: pagoId, init_point: prefResp?.init_point || prefResp?.body?.init_point, reused });
  } catch (err: any) {
