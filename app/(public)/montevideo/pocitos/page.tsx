...
export const metadata = {
  title: "Escorts en Pocitos | Montevideo | VIAVIP",
  description:
    "Encontrá escorts en Pocitos, Montevideo, con perfiles verificados y contacto directo en VIAVIP.",
};

export default async function EscortsPocitosPage({
  searchParams,
}: {
  ...
  const { items, count, error } = await fetchPublicaciones(cat, filtros, { departamento: "Montevideo", zona: "Pocitos" });
  ...
  if (error) {
    console.error("Error cargando Pocitos:", error);
  }
  ...
  return (
    <main>
      ...
      <TrustBlock />
      <DestacadasDiamante categoria={cat} zona="pocitos" />
      <div style={{ padding: "20px 16px 0" }}>
        <h1
          className="vv-section-title"
          style={{ fontSize: "24px", margin: 0 }}
        >
          Escorts VIP en Pocitos
        </h1>
      ...
      <ListadoFiltered
        ...
        queryContext={{ categoria: cat, extra_dep: "Montevideo", extra_zona: "Pocitos" }}
      />
    </main>
  );
}
